...
contract StakingDAO is ReentrancyGuard ,ERC721Holder {
  ...
  mapping(uint256 => uint256) public lockBlocks;
  ...
  function deposit(uint256 _poolId, uint256 _tokenId, address referrer) external nonReentrant noContractAllowed {
    ...
    lockBlocks[_tokenId] = block.number.add(lockTime);
  ...
  function canWithdraw(uint256 _tokenId) public view returns (bool) {
    if (openWithdraw) {
        return true;
    }
    return block.number >= lockBlocks[_tokenId];
  }
  ...
  function withdraw(uint256 _poolId, uint256 _tokenId) public nonReentrant noContractAllowed {
    if (!openWithdraw) {
        require(lockBlocks[_tokenId] < block.number, "dao locked");
    }
